...
METHOD Init( cOptionalPath, lSystemIsOEM ) class frReportManager  
LOCAL oError, nIsOEM

   nIsOEM := IF(lSystemIsOEM <> NIL, IF(lSystemIsOEM, 1, 0), 0)
   ...
   IF ::frSystHandle == 0
      ...
      IF ::frSystHandle <> 0 
         ::PrepareCalls()
         Call_Func_I_I(::_InitSyst, nIsOEM)
...
LOCAL cMessage, cCaption, n

   Call_Proc_(::_FrNotifyError)

   cCaption := iif( oError:severity > ES_WARNING, "[x]Harbour Error", "[x]Harbour Warning" )
   ...
   cMessage := cMessage + Chr(13) + Chr(10) + Chr(13) + Chr(10)

   n := 1
   WHILE ! Empty( ProcName( ++n ) )

      cMessage := cMessage + "Called from " + ProcName( n ) + ;
               "(" + LTrim( Str( ProcLine( n ) ) ) + ")  " + Chr(13) + Chr(10)
   ENDDO
